...
class DatasetClipEmbs(Dataset):
    ...
    def __getitem__(self, index):
        ...
        inv_anno_dict = json.load(open(os.path.join(inverse_path_item, 'inverse_annotation.json')))
        direct_dict = json.load(open(os.path.join(direct_path_item, 'direct_annotation.json')))
        
        '''Evidence Text'''
        captions = self.load_captions(inv_anno_dict) # did not look closely into this function
...
def prepare_data_for_clip(clip_name, device, data_root_path, use_data, batch_size, use_shuffled_data):
    if use_data == "zh" and use_shuffled_data:
        data_items_train = json.load(open(f"{data_root_path}c_train_shuffled.json"))
        data_items_val = json.load(open(f"{data_root_path}c_val_shuffled.json"))
        data_items_test = json.load(open(f"{data_root_path}c_test_shuffled.json"))
    elif use_data == "zh" and not use_shuffled_data:
        data_items_train = json.load(open(f"{data_root_path}c_train.json"))
        data_items_val = json.load(open(f"{data_root_path}c_val.json"))
        data_items_test = json.load(open(f"{data_root_path}c_test.json"))
    

    if use_data == "en":
        data_items_train = json.load(open(f"{data_root_path}e_train.json"))
        data_items_val = json.load(open(f"{data_root_path}e_val.json"))
        data_items_test = json.load(open(f"{data_root_path}e_test.json"))
    
    if use_data == "all":
        data_items_train = json.load(open(f"{data_root_path}dataset_items_train.json"))
        data_items_val = json.load(open(f"{data_root_path}dataset_items_val.json"))
        data_items_test = json.load(open(f"{data_root_path}dataset_items_test.json"))
